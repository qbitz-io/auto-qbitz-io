...
class TestSupervisor(unittest.TestCase):
    def test_api_restart(self):
        # This test simulates starting the supervisor and checks if it restarts the API server on crash
        # We will start the supervisor in a subprocess and then kill the API server process to see if it restarts

        supervisor_process = subprocess.Popen([sys.executable, "backend/supervisor.py"])
        time.sleep(5)  # Wait for API server to start

        # Find the API server process (uvicorn) started by supervisor
        import psutil
        api_process = None
        ...
        # Kill the API server process to simulate crash
